...
def fromDink(sourceData: bytes, game: Game) -> str:
	...
	"""
	scripts = fromDinkToScripts(sourceData, game)
	return fromDinkScriptDictToStrings(scripts)
...
def fromDinkToScripts(sourceData: bytes, game: Game) -> Dict[str, DinkScript]:
	...
	while True:
		...
		_readFunction(dinkReader, scripts, game)
	return scripts
...
def _readFunction(dinkReader: BytesIO, scripts: Dict[str, DinkScript], game: Game) -> DinkFunction:
	...
	blockSize, blockEndOffset = _getBlockInfo(dinkReader, DinkBlockType.MYSTERY_BLOCK_TYPE)
	_checkValue(_MYSTERY_BLOCK_VALUE_SIZE, blockSize, "Unexpected block size of Mystery block")
	_checkValue(_MYSTERY_BLOCK_VALUE_1, Utils.readInt(dinkReader), "Unexpected first value of Mystery block")
	_checkValue(_MYSTERY_BLOCK_VALUE_2, Utils.readShort(dinkReader), "Unexpected second value of Mystery block")

	# Info block, stores script and function names
	dinkFunction: DinkFunction = _parseInfoBlock(dinkReader, scripts, game)
	# Strings block, defines the strings used in the next blocks
	dinkFunction.stringOffsetToString = _parseStringBlock(dinkReader)
	# Variables block
	dinkFunction.variables = _parseVariablesBlock(dinkReader, dinkFunction.stringOffsetToString)
	# Instructions block
	dinkFunction.instructionOperations = _parseInstructionsBlock(dinkReader, dinkFunction)
	# Instruction segments block
	dinkFunction.instructionLines = _parseInstructionSegmentsBlock(dinkReader, dinkFunction)

	# End of the function
	blockSize, blockEndOffset = _getBlockInfo(dinkReader, DinkBlockType.FUNCTION_END)
	if blockSize != 0:
...
def _parseInfoBlock(dinkReader: BytesIO, scripts: Dict[str, DinkScript], game: Game) -> DinkFunction:
	blockSize, blockEndOffset = _getBlockInfo(dinkReader, DinkBlockType.INFO_BLOCK_TYPE)
...
def _parseStringBlock(dinkReader: BytesIO) -> Dict[int, str]:
	blockSize, blockEndOffset = _getBlockInfo(dinkReader, DinkBlockType.STRINGS_BLOCK_TYPE)
...
def _parseVariablesBlock(dinkReader: BytesIO, stringOffsetToString: Dict[int, str]) -> List[Union[int, float, str]]:
	blockSize, blockEndOffset = _getBlockInfo(dinkReader, DinkBlockType.VARIABLES_BLOCK_TYPE)
...
def _parseInstructionsBlock(dinkReader: BytesIO, parentFunction: DinkFunction) -> List[DinkInstructionOperation]:
	blockSize, blockEndOffset = _getBlockInfo(dinkReader, DinkBlockType.INSTRUCTIONS_BLOCK_TYPE)
	instructionList = []
	while dinkReader.tell() < blockEndOffset:
...
def _parseInstructionSegmentsBlock(dinkReader: BytesIO, parentFunction: DinkFunction) -> List[DinkInstructionLine]:
	blockSize, blockEndOffset = _getBlockInfo(dinkReader, DinkBlockType.INSTRUCTION_SEGMENTS_BLOCK_TYPE)
